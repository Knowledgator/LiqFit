# Copyright 2020 The HuggingFace Team and Knowledgator. All rights reserved.
#
...
class ZeroShotClassificationArgumentHandler(ArgumentHandler):
    ...
    def __call__(self, sequences, labels, hypothesis_template, hypothesis_first):
        ...
        if not hypothesis_first:
            ...
            for sequence in sequences:
                sequence_pairs.extend([[hypothesis_template.format(label), sequence] for label in labels])
...
class ZeroShotClassificationPipeline(ChunkPipeline):
    ...
    **much** more flexible.
    ...
    def entailment_id(self):
        if len(self.model.config.label2id.items()) == 0:
            return 0
    ...
    def _parse_and_tokenize(
        ...
        try:
            if encoder_decoder:
                sequence_pairs, decoder_input = sequence_pairs
    ...
    def _sanitize_parameters(self, **kwargs):
        ...
        if "hypothesis_first" in kwargs:
            preprocess_params["hypothesis_first"] = kwargs["hypothesis_first"]
        if "encoder_decoder" in kwargs:
            preprocess_params["encoder_decoder"] = kwargs["encoder_decoder"]
    ...
    def preprocess(self, inputs, candidate_labels=None, hypothesis_template="This example is {}.", hypothesis_first = False, encoder_decoder = False):
        sequence_pairs, sequences = self._args_parser(inputs, candidate_labels, hypothesis_template, hypothesis_first)

        for i, (candidate_label, sequence_pair) in enumerate(zip(candidate_labels, sequence_pairs)):
            model_input = self._parse_and_tokenize(sequence_pair, encoder_decoder = encoder_decoder)
    ...
    def _forward(self, inputs):
        ...
        input_names.extend(['decoder_input_ids', 'decoder_attention_mask'])
        model_inputs = {k: inputs[k] for k in input_names if k in inputs}
    ...
    def postprocess(self, model_outputs, multi_label=False):
        ...
        if multi_label and len(self.model.config.label2id)==0:
            scores = 1 / (1 + np.exp(-entail_contr_logits))

        elif multi_label or len(candidate_labels) == 1:
